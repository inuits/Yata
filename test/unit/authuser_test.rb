...
class AuthuserTest < ActiveSupport::TestCase
  ...
  def test_should_not_rehash_password
    ...
    assert_equal authusers(:quentin), Authuser.authenticate('quentin2', 'monkey')
  end

  def test_should_authenticate_authuser
    assert_equal authusers(:quentin), Authuser.authenticate('quentin', 'monkey')
  end
...
protected
  def create_authuser(options = {})
    record = Authuser.new({ :login => 'quire', :email => 'quire@example.com', :password => 'quire69', :password_confirmation => 'quire69' }.merge(options))
    record.save
    record
  end
end
